  ...
  return (
    <div className="min-h-dvh bg-canvas px-4 py-6 sm:px-6">
      ...
      <div className="border-b border-brand bg-cover bg-center px-5 py-14 text-center" style={site?.coverUrl ? { backgroundImage: `linear-gradient(rgba(8,12,5,.84),rgba(8,12,5,.94)),url(${site.coverUrl})` } : undefined}>
        <p className="ascii-kicker">+-- local_business --+</p>
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {catalog.map((item) => (
                <Card key={item.id} className="flex items-start justify-between gap-3 p-4">
                  <div className="flex min-w-0 gap-3">{item.imageUrl ? <div role="img" aria-label={item.title} className="h-12 w-12 shrink-0 border border-line bg-cover bg-center" style={{ backgroundImage: `url(${item.imageUrl})` }} /> : null}<div><p className="font-medium text-ink">{item.title}</p><p className="text-[11px] uppercase text-brand">{item.category}</p>{item.description ? <p className="mt-1 text-xs text-ink-soft">{item.description}</p> : null}</div></div>
