...
const GOAL_CONFIG: Record<PromoGoal, GoalConfig> = {
  ...
  return_customers: { audience: 'segment', kinds: ['discount'], segments: ['at_risk', 'lapsed', 'declining', 'no_booking', 'birthday_soon'], defaultKind: 'discount', defaultSegment: 'at_risk', defaultValue: 15, placements: ['client_app', 'cashier'] },
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="space-y-4">
        ...
        <Card className="space-y-4">
          ...
          {kind === 'discount' ? <p className="border-l-2 border-brand px-3 text-xs text-ink-soft">Клиент заплатит на {value}% меньше за подходящую покупку. Скидка учитывается на кассе.</p> : null}
